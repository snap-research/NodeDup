...
class Evaluator:
    ...
    def _eval_mrr_valid(self, y_pred_pos, y_pred_neg, type_info, metric):
        '''
            compute mrr
        ...
        if "hits@" in metric:
            ...
            result = 1./ranking_list.to(torch.float)
        elif metric =="auc":
            y_pred_pos = y_pred_pos.view(-1)
            y_pred_neg = y_pred_neg.view(-1)
            true_result = torch.cat((torch.ones(y_pred_pos.size()), torch.zeros(y_pred_neg.size())), dim=0)
            pred_result = torch.cat((y_pred_pos, y_pred_neg), dim=0)
            result = fast_numba_auc_nonw(true_result.cpu().numpy(),pred_result.cpu().numpy())
        # roauc = roc_auc_score(true_result.cpu().numpy(),pred_result.cpu().numpy())
    ...
    def _eval_mrr(self, y_pred_pos, y_pred_neg, type_info):
        ...
        if type_info == 'torch':
            ...
            true_result = torch.cat((torch.ones(y_pred_pos.size()), torch.zeros(y_pred_neg.size())), dim=0)
            pred_result = torch.cat((y_pred_pos, y_pred_neg), dim=0)
            roauc = fast_numba_auc_nonw(true_result.cpu().numpy(),pred_result.cpu().numpy())
            # roauc = roc_auc_score(true_result.cpu().numpy(),pred_result.cpu().numpy())

            return {'hits@10_list': hits10_list,
                     'hits@20_list': hits20_list,
                     'hits@30_list': hits30_list,
