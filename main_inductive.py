...
def main():
    ...
    #     if str(node) not in edge_dict:
    #         if 0 not in results:
    #             results[0] = {}
    #             results[0]['num'] = 1
    #             results[0]['edge_num'] = split_edge['test']['new'][node]["positive"].size(1)
    #         else:
    #             results[0]['num'] +=1
    #             results[0]['edge_num'] += split_edge['test']['new'][node]["positive"].size(1)
    #     elif (edge_dict[str(node)] == 1 or edge_dict[str(node)] == 2):
    #         if "cold" not in results:
    #             results["cold"] = {}
    #             results["cold"]['num'] = 1
    #             results["cold"]['edge_num'] = split_edge['test']['new'][node]["positive"].size(1)
    #         else:
    #             results["cold"]['num'] +=1
    #             results["cold"]['edge_num'] += split_edge['test']['new'][node]["positive"].size(1)
    #     else:
    #         if "warm" not in results:
    #             results["warm"] = {}
    #             results["warm"]['num'] = 1
    #             results["warm"]['edge_num'] = split_edge['test']['new'][node]["positive"].size(1)
    #         else:
    #             results["warm"]['num'] +=1
    #             results["warm"]['edge_num'] += split_edge['test']['new'][node]["positive"].size(1)
    ...
    for run in range(args.runs):
        ...
        # import json
        # file = open(Path(args.dataset_dir) / (args.datasets + "_edge_dict.json"), "r")
        # edge_dict = json.load(file)
        # file.close()
